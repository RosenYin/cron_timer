...
void TestCronTimerInMainThread() {
    ...
    int id = 1;
    // 2023年11月的每秒都会触发
    mgr->AddTimer("* * * * 11 ? 2023", [](void) {Log("--------1 second cron timer hit");}, id);
    // 周一到周日每秒都触发
    mgr->AddTimer("* * * ? * MON-SAT", [](void) {Log(">>>>>>>1 second cron timer hit");}, id+1);
    // 从0秒开始，每3秒钟执行一次
    mgr->AddTimer("0/3 * * * * ?", [](void) {Log("3 second cron timer hit");}, id+1);
    // 1分钟执行一次（每次都在0秒的时候执行）的定时器
    mgr->AddTimer("0 * * * * ?", [](void) {Log("1 minute cron timer hit");}, id+1);
    // 指定时间（15秒、30秒和33秒）点都会执行一次
    mgr->AddTimer("15,30,33 * * * * ?", [](void) {Log("cron timer hit at 15s or 30s or 33s");}, id+1);
    // 指定时间段（40到50内的每一秒）执行的定时器
    mgr->AddTimer("40-50 * * * * ?", [](void) {Log("cron timer hit between 40s to 50s");}, id+1);
    // 每10秒钟执行一次，总共执行3次
